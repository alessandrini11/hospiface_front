...
const index = () => {
	...
	useEffect(() => {
		axios
			.get(`/patients?actualPage=${page || 1}&query=${query || ""}`)
			.then((response) => {
	...
	return (
		<Fragment>
			{created_message && (
				<Alert type="toast" icon="success" message={created_message}></Alert>
			)}
			{error_message && (
				<Alert type="modal" icon="error" title={error_message}></Alert>
			)}
			<CardContainer>
				<div className="">
					<div className="flex justify-between items-center py-4">
						...
						<p className="">
							<Link
								to="/"
								className="inline-block text-green-700 bg-green-300 focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 focus:outline-none">
