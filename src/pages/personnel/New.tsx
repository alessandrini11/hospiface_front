...
const New = (props: Props) => {
    ...
    useEffect(() => {
        axios.get('/medical_services')
            .then(response => {
                const servicesArr: Array<{label: string, value: number}>=[]
                response.data.data.data.forEach((serv: any) =>  {
                    servicesArr.push({label: serv.name, value: serv.id})
                })
                set_services(servicesArr)
    ...
    return (
        <>
            ...
            <form className="space-y-3" onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="">
                    <label htmlFor="serivce" className="block mb-2 text-sm font-medium text-gray-900">Service</label>
                        <Controller
                            name="serivce"
                            control={control}
                            rules={{ required: true }}
